...
# YIELDFILE = "soja-serie-1969-2019_bu_f2010_c_idadjust_delnan.csv"
...
def vis_one_year(year):
    ...
    for i in range(start_row, end_row):
        if not isNaN(yield_np[i, 5]) and not isNaN(yield_np[i, 7]):
            ...
            state = str(state)
            county = str(county)
            dict[mydict[state + '_' + county]] += yield_np[i, -3]
        else:
            print(yield_np[i, 5], yield_np[i, 7], yield_np[i, -3])

    for key, value in dict.items():
        dict[key] = int(dict[key]/10)
...
def plot_map(pred_dict, gdf, title, ):
    ...
    gdf.plot(column='pred2', ax=ax, cmap='YlGn', edgecolor="black", legend=True, scheme='quantiles',
             missing_kwds={"color": "lightgrey", "edgecolor": "darkgrey", "hatch": "///", "label": "Keine Ertragsdaten",
                           })
    plt.rcParams.update({'font.size': 28})
    # plt.title('Durchschnittlicher Ertrag in den Jahren 2010 bis 2019')
    plt.show()
