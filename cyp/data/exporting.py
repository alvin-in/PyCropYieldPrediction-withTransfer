...
class MODISExporter:
    ...
    def __init__(
        ...
        self.locations = load(locations_filepath, usa=True)
    ...
    def export(
        ...
        check_if_done=True,  # ursprünglich False
        ...
        if check_if_done:
            if download_folder is None:
                download_folder = Path("I:/US-SAT-DS") / folder_name
                already_downloaded = get_tif_files(download_folder)

        imgcoll = (
            ee.ImageCollection(self.collection_id)
            .filterBounds(ee.Geometry.Rectangle(-106.5, 50, -64, 23))   # bounds anpassen
            .filterDate("2009-12-31", "2021-12-31")   # date anpassen
        )
