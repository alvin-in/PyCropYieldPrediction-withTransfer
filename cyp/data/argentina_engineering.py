...
class Arg_Engineer:
    ...
    def __init__(
        ...
        yield_data_filepath=Path("data/soja-serie-1969-2019(3).csv"),
        county_data_filepath=Path("data/departamentos.csv"),
    ):
        ...
        yield_data = load(yield_data_filepath)[
            ["anio", "provincia_id", "departamento_id", "redimiento_buxacre"]
        ]
        yield_data.columns = ["anio", "provincia_id", "departamento_id", "redimiento_buxacre"]
        county_data = pd.read_csv(county_data_filepath)[
            ["departamento_id", "provincia_id", "departamentos__centroide__lon", "departamentos__centroide__lat"]
        ]
        county_data.columns = ["departamento_id", "provincia_id", "departamentos__centroide__lon", "departamentos__centroide__lat"]
        self.yield_data = yield_data.merge(
            county_data, how="left", on=["departamento_id", "provincia_id"]
        )
    ...
    @staticmethod
    def filter_timespan(imcol, start_day=0, end_day=365, composite_period=8, bands=9):
        """
    ...
    def process(
        ...
        for yield_data in self.yield_data.itertuples():
            year = yield_data.anio
            county = yield_data.departamento_id
            state = yield_data.provincia_id

            filename = f"{year}_{int(state)}_{int(county)}.npy"
            if filename in self.processed_files:
                image = np.load(self.cleaned_data / filename)
                image = self.filter_timespan(
                    image, start_day=81, end_day=185, bands=num_bands    # was 49 to 305
                )
                ...
                yields.append(yield_data.redimiento_buxacre)
                years.append(year)
                ...
                try:
                    lat, lon = float(yield_data.departamentos__centroide__lat), float(yield_data.departamentos__centroide__lon)
                except ValueError:
                    lat = float(yield_data.departamentos__centroide__lat[:-1])
                    lon = -float(yield_data.departamentos__centroide__lon)
