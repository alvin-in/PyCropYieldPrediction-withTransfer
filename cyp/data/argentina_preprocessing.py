...
class ArgDataCleaner:
    ...
    def __init__(
            self,
            mask_path=Path("data/cover"),
            temperature_path=Path("data/temp"),
            image_path=Path("data/sat"),
            yield_data_path=Path("data/soja-serie-1969-2019(3).csv"),
            savedir=Path("data/arg_im_out"),
        ...
        self.yield_data = load(yield_data_path)[
            ["anio", "provincia_id", "departamento_id"]
        ].values

    def process(self, num_years=10, delete_when_done=False, checkpoint=True):
        """
...
def process_county(
    ...
    if checkpoint & (len(list(savedir.glob(f"*_{state}_{county}.npy"))) == num_years-1):
    ...
    prefix = filename.split(".")[0].split("-")[0]  # fraglich, da '-' in Dateinnamen; Check nach eigener Umbenennung
    ...
    start_year = 2010  # newest dataset begins in 2010 # 2003: start year from the MODIS website
...
def divide_into_years(img, bands, composite_period, num_years=10, extend=False, shift=False):
    ...
    image_list = []
    # print(math.ceil(2/3 * math.ceil(365 / composite_period)) * bands)
    cur_idx = math.ceil(2/3 * math.ceil(365 / composite_period)) * bands
    if shift is True:
        cur_idx = 0
    for i in range(0, num_years - 1):   # hier -2?
        image_list.append(img[:, :, cur_idx: cur_idx + bands_per_year])
        cur_idx += bands_per_year
    image_list.append(img[:, :, cur_idx:])  # hier Eingriff?
    return image_list
