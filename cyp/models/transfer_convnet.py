...
class TransConvModel(TransferBase):
    """
    ...
    def reinitialize_model(self, predict_year, time=None, freeze=0, us_init=False):
        ...
        # appended for shorter time periods: need to check up
        if self.device.type != "cpu":
            model = model.cuda()
        self.model = model

        init_model = "us_with2020"  # "us_20000_hyp"
        # wandb.config = {"init_model": init_model}
        ...
        if us_init:
            ...
            #    + str(predict_year) + "_1_32_gp.pth.tar"
            #))["state_dict"])
            model.convblocks = model2.convblocks

        # Freezes the first freeze'th convolution layers of the model with params of the given model.
        if freeze > 0:
            if freeze > 6:
                print("Warning! The model has only 6 layers of convolution. In the following, all 6 will be frozen.")
                freeze = 6
            model2 = copy.deepcopy(self.model)
            model2.load_state_dict(torch.load(Path(
                "data\\" + init_model + "\\cnn\\"
            ...
            # model2.load_state_dict(torch.load(Path(
            #    "H:\\BA\\pycrop-yield-prediction\\data\\arg_models_us_2013-2019_57-297\\cnn\\2015_1_32_gp.pth.tar"
            # ))["state_dict"])

            for i in range(freeze):
                model.convblocks[i] = model2.convblocks[i]
                model.convblocks[i].requires_grad_(False)
...
class ConvNet(nn.Module):
    ...
    def __init__(self, in_channels=9, dropout=0.5, dense_features=None, time=32):
        ...
        self.convblocks = nn.ModuleList(
            [
                ConvBlock(
                    ...
                    dropout=dropout,
                )  # .requires_grad_(False)     # turn off autograd to freeze Convolution
                for i in range(1, len(stride_list))
            ]
        )

        # for i in range(3):
        #    self.convblocks[i].requires_grad_(False)  # turn off autograd to freeze part of Conv
        ...
        # pretrained_dict = torch.load(Path("data\\models\\cnn\\2014_2_32_gp.pth.tar"))
    ...
    def forward(self, x, return_last_dense=False):
        ...
        """
        layer_output_target = {}    # added for DELTA
        for block in self.convblocks:
            x = block(x)
            layer_output_target[block] = x  # added for DELTA
        ...
        for layer_number, dense_layer in enumerate(self.dense_layers):
            x = dense_layer(x)
            # if layer_number == len(self.dense_layers) - 2:
            #     y = x
            if return_last_dense and (layer_number == len(self.dense_layers) - 2):
                output = x
        if return_last_dense:
            return x, y, output, layer_output_target
        return x, y, layer_output_target
