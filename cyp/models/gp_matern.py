...
class GaussianProcess:
    ...
    def __init__(self, sigma=1, r_loc=0.5, r_year=1.5, sigma_e=0.32, sigma_b=0.01, nu=1.5):
        self.sigma = sigma
        self.r_loc = r_loc
        self.r_year = r_year
    ...
    def run(
        ...
        ### hier hinzugefügt
        K = Matern(nu=self.nu, length_scale=self.r_loc)
        covMat_loc = K(locations)
        K = Matern(nu=self.nu, length_scale=self.r_year)
        covMat_year = K(years)

        # Kernel abgeändert covMat statt np.exp(-se_loc)
        kernel = ((self.sigma ** 2) * covMat_loc * covMat_year) + noise
